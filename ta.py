...
#from statsmodels import moments
import pandas as pd
...
def aroon(s, n=25):
    up = 100 * (s.high).rolling(n + 1).apply(lambda x: x.argmax()) / n
    dn = 100 * (s.low).rolling(n + 1).apply(lambda x: x.argmin()) / n
...
def cci(s, n=20, c=0.015):
    ...
    mdev = s.rolling(n).apply(lambda x: np.fabs(x - x.mean()).mean())
...
def safezone(s, position, nmean=10, npen=2.0, nagg=3):
    ...
    pen = (sgn*s.diff()).rolling(nmean).apply(
